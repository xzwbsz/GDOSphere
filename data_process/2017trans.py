...
for time in tqdm(date_list): #每天的5个变量，37层
    filename = main+'/'+str(year)+'/'+time.strftime("ERA5_plevel_%Y_%m_%d_%H.nc")
    AA = xr.open_dataset(filename)
    inputing = np.array(AA.to_array())
    # print(inputing.shape)    
    Ff = torch.tensor(inputing.squeeze()).unsqueeze(0)
    NEW_file = pre_load_target(Ff,edge_src_target, edge_weight, nshape_edge)
    NEW_file.squeeze()
    NEW_file = np.array(NEW_file.cpu()).squeeze()
    file_name = time.strftime("ERA5_plevel_%Y_%m_%d_%H")
